...
- `.mise/tasks/` — the tasks themselves, as executable files (`format`, `clean`, `build/{_default,debug,release}`, `test/{_default,rust,tun,tproxy,ebpf}`, `demo/{_default,bwrap}`); not configured in `mise.toml`
