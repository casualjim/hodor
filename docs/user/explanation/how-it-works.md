...
**Transparent capture (Linux).** `--proxy-backend` selects a backend. `tproxy` installs an `IP_TRANSPARENT` listener plus nftables rules and policy routes over netlink, so the kernel redirects every outbound TCP connection in the namespace to hodor; UDP passes through and QUIC on port 443 is dropped so HTTP/3 clients fall back to TCP. `tun` opens a TUN device and feeds it into a userspace stack (smoltcp, medium-ip, any-ip), with policy routes in a dedicated table pointing at the device, and handles UDP itself: DNS relayed to the system resolver, QUIC dropped, other flows relayed to their destination. `ebpf` attaches three programs to a cgroup over `bpf(2)` — a `connect4` hook that records the real destination in a map keyed by socket cookie and rewrites the address to hodor's loopback listener, an egress hook that indexes the redirected flow by its local 4-tuple so the accepted connection can be resolved back, and a `recvmsg4` hook that restores the original source on relayed UDP replies. No netfilter, no policy routes, no `IP_TRANSPARENT`; the attach handles belong to the process, so exit detaches the programs. All three are peers — same interception contract, different mechanism — and in all three the client configured nothing and cannot bypass the proxy. hodor keeps its own upstream dials out of its own capture: a fwmark for `tproxy` and `tun`, cgroup membership plus a PID comparison for `ebpf`.
