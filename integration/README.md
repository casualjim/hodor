...
`--features tun` binary) in the deployment shape hodor exists for: the
client container shares hodor's network namespace and knows nothing about
any proxy. hodor runs `serve --tun`, installs its policy routing inside the
shared netns, and every client connection is captured on the way out.
...
- `hodor` — legs on the default bridge and the `server` network;
  `cap_add: NET_ADMIN` + `/dev/net/tun` for TUN capture and rtnetlink
  routes/rules. Marked upstream dials bypass capture (fwmark rule).
...
Scenario 3's grant host is the literal destination IP because TUN raw TCP
has no SNI — the destination address is the identity (`src/tun/mod.rs`
`tun_conn_task`). Scenario 5 is the fail-closed control: without a grant
...
The task builds the release binary with the `tun` feature, packs it into
