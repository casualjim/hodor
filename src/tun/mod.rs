...
use crate::proxy::{ProxyState, serve_candidate_stream};
use chan::ChanStream;
use classify::{IpPacket, classify_packet};
use phy::{TUN_MTU, TunPhy};
pub(crate) use route::RouteGuard;
#[cfg(test)]
use route::{RoutePlan, TABLE_MAIN, Undo};
use route::{add_route, add_rule, capture_routes, capture_rules, capture_undos, link_index, netlink};
use tracker::{NewTcpConn, TaskMsg, TcpTracker};
use udp::{MAX_UDP_SESSIONS, UdpKey, udp_session_task, upstream_dns};
...
fn ingress(ctx: Ingress<'_, '_>) {
  ...
  match classify_packet(packet) {
    ...
    IpPacket::Udp { src, dst, payload } => {
      if dst.port() == 443 {
        return; // QUIC deflect: force TCP/TLS fallback
      }
...
mod tests {
  ...
  use crate::grants::ResolvedConfig;
  ...
  fn test_state(grants: Vec<crate::grants::Grant>, fwmark: Option<u32>) -> Arc<ProxyState> {
    crate::ca::install_crypto_provider();
    let ca = crate::ca::CertAuthority::generate().unwrap();
  ...
  async fn tun_live_tcp_mitm_substitutes() {
    ...
    let stub_cert = crate::ca::generate_domain_cert(SNI, &ca).unwrap();
    ...
    let grants = vec![crate::grants::Grant {
      label: "t".into(),
      fake: FAKE.into(),
      value: secrecy::SecretString::from(VALUE),
      allow: vec![crate::grants::UriGrant {
        scheme: crate::grants::Scheme::Https,
